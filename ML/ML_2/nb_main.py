...
def main(as_grayscale=False):
    print(f"\nRunning Gaussian Naive Bayes model with {'grayscale' if as_grayscale else 'RGB'} images.")
    ...
    # This step is necessary for Gaussian Naive Bayes since it expects 2D data (samples x features)
    ...
    print("Initializing and fitting the Gaussian Naive Bayes model...")
    gnb = GaussianNaiveBayes()         # Create an instance of the Gaussian Naive Bayes model
    gnb.fit(X_train, y_train)          # Fit the model to the training data
    print("Model fitting completed.")

    # Predict and evaluate the model
    print("Making predictions...")
    predictions = gnb.predict(X_test)  # Predict the labels for the test data
    print("Predictions completed.")

    # Calculate and print accuracy
    accuracy = accuracy_score(y_test, predictions)  # Compute the accuracy of the model
    print(f"Gaussian Naive Bayes Accuracy ({'Grayscale' if as_grayscale else 'RGB'}): {accuracy}")  # Print the accuracy
